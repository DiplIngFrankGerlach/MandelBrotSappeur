...
class OberBMP_Erzeuger: public SPRObject {
public:
void addRef(){SPRObject::addRef();}
void release(){SPRObject::release();}
virtual SPRObject* getSPRObject(){return this;}
int _breite;
int _hoehe;
virtual void erzeugeAnfang(char* stackStartPtr);
virtual void zeichne(char* stackStartPtr,int x,int y,int color_bgr);
virtual int pufferGroesse(char* stackStartPtr);
virtual int schreibe(char* stackStartPtr,SPRFile& ausgabe);
int breite(char* stackStartPtr);
int hoehe(char* stackStartPtr);
virtual void destroy_sappeur_internal();
void* operator new(size_t objectSize,void* place);
};

class BMP_Erzeuger: public OberBMP_Erzeuger {
public:
void addRef(){SPRObject::addRef();}
void release(){SPRObject::release();}
virtual SPRObject* getSPRObject(){return this;}
SPRSmartPtr<SPRArray<char> > bildPuffer;
int _breiteZeileOktets;
PrintfClass pfc;
BMP_Erzeuger(char* stackStartPtr,int groesseX,int groesseY);
virtual void erzeugeAnfang(char* stackStartPtr);
void erzeugeSinus(char* stackStartPtr);
void erzeugeAusAbbildung(char* stackStartPtr);
void erzeugeAusAbbildung(char* stackStartPtr,String_16& abbildung,SPRSmartPtr<SPRArray<char> >& bp,int& groessePuffer);
virtual void zeichne(char* stackStartPtr,int x,int y,int color_bgr);
void writeUI32(char* stackStartPtr,int& pos,int value);
virtual int schreibe(char* stackStartPtr,SPRFile& ausgabe);
virtual void destroy_sappeur_internal();
void* operator new(size_t objectSize,void* place);
};
...
int WIDTH;
int HEIGHT;
double mandelbrotSmooth(char* stackStartPtr,Complex& c);
void hsvToRgb(char* stackStartPtr,double h,double s,double v,int& r,int& g,int& b);
void pixelToComplex(char* stackStartPtr,int x,int y,Complex& result);
